use qs_backend::core::enums::IPCModule;
use qs_backend::ipc::client::{ClientRequest, ResponseEvent};
use qs_backend::modules::network::action::NetworkAction;
use serde_json::json;
...
async fn test_client_request_serialization() {
    ...
    let request = ClientRequest::Network(NetworkAction::Connect {
...
async fn test_client_request_deserialization() {
    ...
    assert!(matches!(
        request,
        ClientRequest::Network(NetworkAction::ScanWifi)
    ));
...
async fn test_response_event_deserialization() {
    ...
    let response: ResponseEvent<serde_json::Value> =
