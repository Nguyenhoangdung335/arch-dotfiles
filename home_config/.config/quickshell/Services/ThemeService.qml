...
Singleton {
  ...
  function parseEntries(data: string) {
    ...
    for (const fileName of lines) {
      if (!fileName || !fileName.endsWith(".qml") || fileName === "qmldir")
        continue;
    ...
    if (Cfg.Global.isDebug) {
      Log.info("ThemeService: Loaded", tempModelList.length, "themes");
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      onStreamFinished: {
        if (Cfg.Global.isDebug)
          Log.debug("ThemeService: Scanned directory", root.palettesPath);
    ...
    stderr: SplitParser {
      onRead: data => {
        Log.error("ThemeService: Error scanning palettes directory", data);
      }
    }
  }
}
