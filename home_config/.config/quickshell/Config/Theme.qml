  ...
  FileView {
    ...
    onLoadFailed: err => {
      if (err === FileViewError.FileNotFound) {
        ...
        Log.error("Theme config file not found, creating default at " + root.configPath);
    ...
    JsonAdapter {
      ...
      onPaletteFamilyChanged: if (paletteFamily)
        root.paletteFamily = paletteFamily
      onPaletteVariantChanged: if (paletteVariant)
        root.paletteVariant = paletteVariant
      onWallpaperDirChanged: if (wallpaperDir)
        root.wallpaperDir = wallpaperDir
    }
  }
}
